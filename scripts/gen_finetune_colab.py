...
cells.append(code(
    ...
    "    \"\"\"candidates: list of entity strings; facts: list of triple strings;\n"
    "    gold_set: set of correct entity strings.\"\"\"\n"
    "    correct_idx = -1\n"
    "    for i, c in enumerate(candidates):\n"
    "        if c in gold_set:\n"
    "            correct_idx = i; break\n"
    "    if correct_idx < 0:\n"
    "        return None  # retrieval miss, skip\n"
    ...
    "    \"\"\"Run QUEST-KG inference but harvest the top-K candidate tails and supporting triples.\"\"\"\n"
    ...
    "    facts = []\n"
    "    fact_seen = set()\n"
    "    for p in paths[:MAX_CANDIDATES * 2]:  # over-retrieve, dedupe\n"
    ...
    "    print(f'\\n=== {ds_name}: building train data ===')\n"
    "    ds_train = load_dataset(ds_name, DATA_ROOT)\n"
    "    # rmanluo train/test split is by query index; we use the FIRST chunk as train\n"
    "    # (this is the standard convention used by RoG / GNN-RAG / ChatKBQA).\n"
    ...
    "            pass\n"
    "        if (i+1) % 100 == 0:\n"
    "            print(f'  {i+1}/{len(train_queries)}  kept={len(train_data[ds_name])}  ({(time.perf_counter()-t0):.1f}s)')\n"
    "    print(f'  {ds_name} train: {len(train_data[ds_name])} examples')\n"
    "\n"
    "    # Test data: same retrieval pass on first N_TEST queries\n"
    ...
    "        except Exception as e:\n"
    "            pass\n"
    "    print(f'  {ds_name} test: {len(test_data[ds_name])} examples')\n"
    "\n"
    "# Save\n"
    "import json\n"
    ...
    "            f.write(json.dumps(ex) + '\\n')\n"
    "print('saved finetune_data/*.jsonl')"
))
...
cells.append(md("## 9. Inference + scoring"))
cells.append(code(
    ...
    "        rows.append(dict(qid=qi, pred=pred, gold=ex['gold'], em=int(is_correct), raw=out[:120]))\n"
    "        if (qi+1) % 50 == 0:\n"
    "            print(f'  {ds_name} {qi+1}/{len(cells)}  H@1 so far: {correct/(qi+1):.3f}')\n"
    "    h1 = correct / max(len(cells), 1)\n"
    "    return h1, rows\n\n"
    "results = {}\n"
    "for ds in DATASETS:\n"
    "    h1, rows = score_dataset(ds)\n"
    "    results[ds] = dict(hits_at_1=h1, n=len(rows), per_query=rows)\n"
    "    print(f'\\n=== {ds}: Hits@1 = {h1:.4f}  (n={len(rows)}) ===')"
))
